...
const Navigation = () => {
  ...
  return (
    <nav className={classNames('menu', { 'menu--open': isMenuOpen })}>
      <h1 className="menu__title">Weather App</h1>
      <form className="menu__form" onSubmit={handleOnFormSubmit}>
        <label className="menu__label" htmlFor="city">
          <img className="menu__icon" src={SearchIcon} alt="search icon" />
          <input
        ...
        <input className="menu__input menu__input--btn" type="submit" value="Search" />
        {isError ? <p className="menu__error">the city was not found</p> : null}
      </form>
      <ul className="menu__list-container">{weathersListMap}</ul>
    </nav>
  );
};
